...
{
    public static class SpawnSummonDataExtensions
    {
        // FIXME(api): Move to utils
        public static FServantReq ToGame(this SpawnSummonData value, WukongPawnState pawnState)
        {
            BGW_PreloadAssetMgr preloadAssetMgr = BGW_PreloadAssetMgr.Get(GameUtils.GetWorld());

            var summoner = pawnState.GetPawnByNetworkId(value.SummonerNetId);
            var catchTarget = pawnState.GetPawnByNetworkId(value.CatchTargetNetId);
        ...
        public static SpawnSummonData FromGame(this FServantReq value, WukongPawnState pawnState)
        {
            var summonerNetId = pawnState.GetNetworkIdByActor(value.Summoner);
            var catchTargetNetId = pawnState.GetNetworkIdByActor(value.CatchTarget);
            ...
            return new SpawnSummonData()
            {
                SummonerNetId = summonerNetId ?? default,
                ...
                CooperativeSCGuid = value.CooperativeSCGuid,
                AliveTime = value.AliveTime,
                CatchTargetNetId = catchTargetNetId ?? default,
